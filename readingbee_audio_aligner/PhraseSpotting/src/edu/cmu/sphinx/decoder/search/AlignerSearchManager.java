...
public class AlignerSearchManager extends TokenSearchManager {
	...
	private boolean phraseDetected;
	 ...
	 */
	public AlignerSearchManager(LogMath logMath, Linguist linguist,
			Pruner pruner, AcousticScorer scorer,
			ActiveListFactory activeListFactory, boolean showTokenCount,
	...
	public void newProperties(PropertySheet ps) throws PropertyException {
		...
		double relativeWordBeamWidth = ps
				.getDouble(PROP_RELATIVE_WORD_BEAM_WIDTH);
	...
	protected void growBranches() {
		...
		if (logger.isLoggable(Level.FINE)) {
			...
			totalHmms += hmms;
			logger.fine("Frame: " + currentFrameNumber + " Hmms: " + hmms
					+ "  total " + totalHmms);
		}
	}
		...
		for (SearchStateArc arc : arcs) {
			...
			if (firstToken || bestToken.getScore() <= logEntryScore) {
				Token newToken = new Token(predecessor, nextState,
						logEntryScore, arc.getInsertionProbability(), arc
								.getLanguageProbability(), currentFrameNumber);
				tokensCreated.value++;
	...
	private boolean spotterContains(float time) {
	...
	protected void processPhrase() {
		StringTokenizer st = new StringTokenizer(phrase);
		phraseWordList = new LinkedList<String>();
		while (st.hasMoreTokens()) {
			phraseWordList.add(st.nextToken());
		}
	}

	// Take Spotter's result and convert into a tangible type for pruning
	public void setSpotterResult(
			List<edu.cmu.sphinx.phrasespotter.Result> result) {
